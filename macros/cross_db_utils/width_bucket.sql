{% macro width_bucket(expr, min_value, max_value, num_buckets) %}
  {{ return(adapter.dispatch('width_bucket', 'dbt_utils_justos') (expr, min_value, max_value, num_buckets)) }}
{% endmacro %}
...
{% macro default__width_bucket(expr, min_value, max_value, num_buckets) -%}
    ...
    (
        ...
        case
            when
                mod(
                    {{ dbt_utils_justos.safe_cast(expr, dbt_utils_justos.type_numeric() ) }},
                    {{ dbt_utils_justos.safe_cast(bin_size, dbt_utils_justos.type_numeric() ) }}
                ) = 0
            then 1
            else 0
        end
