{% macro get_period_boundaries(target_schema, target_table, timestamp_field, start_date, stop_date, period) -%}
    {{ return(adapter.dispatch('get_period_boundaries', 'dbt_utils_justos')(target_schema, target_table, timestamp_field, start_date, stop_date, period)) }}
{% endmacro %}
  ...
  {% call statement('period_boundaries', fetch_result=True) -%}
    with data as (
      select
          ...
          coalesce(
            {{dbt_utils_justos.dateadd('millisecond',
                                -1,
    ...
    select
      start_timestamp,
      stop_timestamp,
      {{dbt_utils_justos.datediff('start_timestamp',
                           'stop_timestamp',
...
{% macro get_period_sql(target_cols_csv, sql, timestamp_field, period, start_timestamp, stop_timestamp, offset) -%}
    {{ return(adapter.dispatch('get_period_sql', 'dbt_utils_justos')(target_cols_csv, sql, timestamp_field, period, start_timestamp, stop_timestamp, offset)) }}
{% endmacro %}
...
{% materialization insert_by_period, default -%}
  ...
  {% set _ = dbt_utils_justos.get_period_boundaries(schema,
                                              identifier,
                                              timestamp_field,
                                              start_date,
                                              stop_date,
  ...
  {% for i in range(num_periods) -%}
    ...
    {{ dbt_utils_justos.log_info(msg) }}

    {%- set tmp_identifier = model['name'] ~ '__dbt_incremental_period' ~ i ~ '_tmp' -%}
    {%- set tmp_relation = api.Relation.create(identifier=tmp_identifier,
                                               schema=None, type='table') -%}
    {% call statement() -%}
      {% set tmp_table_sql = dbt_utils_justos.get_period_sql(target_cols_csv,
                                                       sql,
                                                       timestamp_field,
                                                       period,
                                                       start_timestamp,
