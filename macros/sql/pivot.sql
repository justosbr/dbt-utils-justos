...
Example:
    ...
    select
      size,
      {{ dbt_utils_justos.pivot('color', dbt_utils_justos.get_column_values('public.test',
                                                              'color')) }}
    from public.test
    group by size
...
{% macro pivot(column,
               ...
               distinct=False) %}
    {{ return(adapter.dispatch('pivot', 'dbt_utils_justos')(column, values, alias, agg, cmp, prefix, suffix, then_value, else_value, quote_identifiers, distinct)) }}
{% endmacro %}
...
{% macro default__pivot(column,
  ...
  {% for v in values %}
    ...
    {% if alias %}
      ...
      {% else %}
        as {{ dbt_utils_justos.slugify(prefix ~ v ~ suffix) }}
      {% endif %}
    {% endif %}
