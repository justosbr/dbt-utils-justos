...
{% macro haversine_distance(lat1, lon1, lat2, lon2, unit='mi') -%}
    {{ return(adapter.dispatch('haversine_distance', 'dbt_utils_justos')(lat1,lon1,lat2,lon2,unit)) }}
{% endmacro %}
...
{% set radians_lat1 = dbt_utils_justos.degrees_to_radians(lat1) %}
{% set radians_lat2 = dbt_utils_justos.degrees_to_radians(lat2) %}
{% set radians_lon1 = dbt_utils_justos.degrees_to_radians(lon1) %}
{% set radians_lon2 = dbt_utils_justos.degrees_to_radians(lon2) %}
{%- if unit == 'mi' %}
    {% set conversion_rate = 1 %}
{% elif unit == 'km' %}
    {% set conversion_rate = 1.60934 %}
