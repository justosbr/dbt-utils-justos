...
{% macro unpivot(relation=none, cast_to='varchar', exclude=none, remove=none, field_name='field_name', value_name='value', table=none) -%}
    {{ return(adapter.dispatch('unpivot', 'dbt_utils_justos')(relation, cast_to, exclude, remove, field_name, value_name, table)) }}
{% endmacro %}
...
{% macro default__unpivot(relation=none, cast_to='varchar', exclude=none, remove=none, field_name='field_name', value_name='value', table=none) -%}
  ...
  {%- do dbt_utils_justos._is_relation(relation, 'unpivot') -%}
  {%- do dbt_utils_justos._is_ephemeral(relation, 'unpivot') -%}
  ...
  {%- for col in include_cols -%}
    select
      ...
      cast('{{ col.column }}' as {{ dbt_utils_justos.type_string() }}) as {{ field_name }},
      cast(  {% if col.data_type == 'boolean' %}
           {{ dbt_utils_justos.cast_bool_to_text(col.column) }}
             {% else %}
           {{ col.column }}
             {% endif %}
