...
- Allow user to provide any case type when defining the `exclude` argument in `dbt_utils_justos.star()` ([#403](https://github.com/dbt-labs/dbt-utils/pull/403))
...
- make `sequential_values` schema test use `dbt_utils_justos.type_timestamp()` to allow for compatibility with db's without timestamp data type. [#376](https://github.com/fishtown-analytics/dbt-utils/pull/376) from [@swanderz](https://github.com/swanderz)
...
* Switch usage of `adapter_macro` to `adapter.dispatch`, and define `dbt_utils_justos_dispatch_list`,
