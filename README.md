...
# dbt-utils for Justos

## Installation Instructions
Check [dbt Hub](https://hub.getdbt.com/dbt-labs/dbt_utils_justos/latest/) for the latest installation instructions, or [read the docs](https://docs.getdbt.com/docs/package-management) for more information on installing packages.
...
models:
  - name: model_name
    tests:
      - dbt_utils_justos.equal_rowcount:
...
models:
  - name: model_name
    tests:
      - dbt_utils_justos.fewer_rows_than:
          compare_model: ref('other_table_name')
```
...
models:
  - name: model_name
    tests:
      - dbt_utils_justos.expression_is_true:
...
models:
  - name: model_name
    columns:
      - name: from_column
        tests:
          - dbt_utils_justos.cardinality_equality:
...
models:
  - name: my_model
    columns:
      - name: id
        tests:
          - dbt_utils_justos.not_null_where:
              where: "_deleted = false"
```
...
models:
  - name: my_model
    columns:
      - name: id
        tests:
          - dbt_utils_justos.not_null_proportion:
              at_least: 0.95
```
...
models:
  - name: my_model
    columns:
      - name: city
        tests:
          - dbt_utils_justos.not_accepted_values:
...
models:
  - name: model_name
    columns:
      - name: id
        tests:
          - dbt_utils_justos.relationships_where:
              to: ref('other_model_name')
              field: client_id
...
models:
  ...
  - name: age_brackets
    tests:
      - dbt_utils_justos.mutually_exclusive_ranges:
...
seeds:
  - name: util_even_numbers
    columns:
      - name: i
        tests:
          - dbt_utils_justos.sequential_values:
              interval: 2
...
- name: revenue_by_product_by_month
  tests:
    - dbt_utils_justos.unique_combination_of_columns:
...
models:
  - name: model_name
    columns:
      - name: user_id
        tests:
          - dbt_utils_justos.accepted_range:
              min_value: 0
              inclusive: false
...
{% set payment_methods = dbt_utils_justos.get_column_values(table=ref('stg_payments'), column='payment_method') %}
...
{% set payment_methods = dbt_utils_justos.get_column_values(
...
{% set relations = dbt_utils_justos.get_relations_by_pattern('schema_pattern%', 'table_pattern') %}
...
{% set relations = dbt_utils_justos.get_relations_by_pattern('schema_pattern', 'table_pattern%', '%deprecated') %}

-- Example using the union_relations macro
{% set event_relations = dbt_utils_justos.get_relations_by_pattern('venue%', 'clicks') %}
{{ dbt_utils_justos.union_relations(relations = event_relations) }}
```
...
```sql
{% set relations_to_drop = dbt_utils_justos.get_relations_by_pattern(
...
{% set relations = dbt_utils_justos.get_relations_by_prefix('my_schema', 'my_prefix') %}

-- Returns a list of relations as above, excluding any that end in `deprecated`
{% set relations = dbt_utils_justos.get_relations_by_prefix('my_schema', 'my_prefix', '%deprecated') %}

-- Example using the union_relations macro
{% set event_relations = dbt_utils_justos.get_relations_by_prefix('events', 'event_') %}
{{ dbt_utils_justos.union_relations(relations = event_relations) }}
```
...
{% set california_cities = dbt_utils_justos.get_query_results_as_dict("select * from" ~ ref('cities') ~ "where state = 'CA' and city is not null ") %}
select
  city,
...
```
{{ dbt_utils_justos.haversine_distance(48.864716, 2.349014, 52.379189, 4.899431) }}

{{ dbt_utils_justos.haversine_distance(
    lat1=48.864716,
    lon1=2.349014,
    lat2=52.379189,
    lon2=4.899431,
...
```sql
select
  {{ dbt_utils_justos.star(ref('my_model')) }}
from {{ ref('my_model') }}
...
```sql
select
{{ dbt_utils_justos.star(from=ref('my_model'), except=["exclude_field_1", "exclude_field_2"]) }}
from {{ ref('my_model') }}
...
**Usage:**
```
{{ dbt_utils_justos.generate_series(upper_bound=1000) }}
```
...
**Usage:**
```
{{ dbt_utils_justos.surrogate_key(['field_a', 'field_b'[,...]]) }}
```
...
**Usage:**
```
{{ dbt_utils_justos.safe_add('field_a', 'field_b'[,...]) }}
```
...
**Usage:**
```
{{ dbt_utils_justos.pivot(<column>, <list of values>) }}
```
...
**Example:**
    ...
    select
      size,
      {{ dbt_utils_justos.pivot(
          'color',
          dbt_utils_justos.get_column_values(ref('orders'), 'color')
      ) }}
    from {{ ref('orders') }}
    group by size
...
**Usage:**
    ...
    {{ dbt_utils_justos.unpivot(ref('orders'), cast_to='varchar', exclude=['date','status']) }}
...
**Usage:**
```
{{ dbt_utils_justos.get_url_parameter(field='page_url', url_parameter='utm_source') }}
```
...
**Usage:**
```
{{ dbt_utils_justos.get_url_host(field='page_url') }}
```
...
**Usage:**
```
{{ dbt_utils_justos.get_url_path(field='page_url') }}
```
----
...
**Usage:**
```
{{ dbt_utils_justos.dateadd(datepart='day', interval=1, from_date_or_timestamp="'2017-01-01'") }}
```
...
**Usage:**
```
{{ dbt_utils_justos.datediff("'2018-01-01'", "'2018-01-20'", 'day') }}
```
...
**Usage:**
```
{{ dbt_utils_justos.split_part(string_text='1,2,3', delimiter_text=',', part_number=1) }}
```
...
**Usage:**
```
{{ dbt_utils_justos.date_trunc(datepart, date) }}
```
...
**Usage:**
```
{{ dbt_utils_justos.last_day(date, datepart) }}
```
...
**Usage:**
```
{{ dbt_utils_justos.width_bucket(expr, min_value, max_value, num_buckets) }}
```
...
{{ dbt_utils_justos.pretty_time() }}

{#- This will return a string like '2019-05-02 14:50:34' -#}
{{ dbt_utils_justos.pretty_time(format='%Y-%m-%d %H:%M:%S') }}
```
...
{{ dbt_utils_justos.pretty_log_format("my pretty message") }}
```
...
```sql
{{ dbt_utils_justos.log_info("my pretty message") }}
```
...
- Users who wish to override a low-lying `dbt_utils_justos` macro with a custom implementation, and have that implementation used by other `dbt_utils_justos` macros
...
dbt v0.20.0 introduced a new project-level `dispatch` config that enables an "override" setting for all dispatched macros. If you set this config in your project, when dbt searches for implementations of a macro in the `dbt_utils_justos` namespace, it will search through your list of packages instead of just looking in the `dbt_utils_justos` package.

Set the config in `dbt_project.yml`:
```yml
dispatch:
  - macro_namespace: dbt_utils_justos
    search_order:
      ...
      - dbt_utils_justos                  # always include dbt_utils_justos as the last place to search
```

If overriding a dispatched macro with a custom implementation in your own project's `macros/` directory, you must name your custom macro with a prefix: either `default__` (note the two underscores), or the name of your adapter followed by two underscores. For example, if you're running on Postgres and wish to override the behavior of `dbt_utils_justos.datediff` (such that `dbt_utils_justos.date_spine` will use your version instead), you can do this by defining a macro called either `default__datediff` or `postgres__datediff`.

Let's say we have the config defined above, and we're running on Spark. When dbt goes to dispatch `dbt_utils_justos.datediff`, it will search for macros the following in order:
```
...
dbt_utils_justos.spark__datediff
dbt_utils_justos.default__datediff
```
